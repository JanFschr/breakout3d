...
export function validateLevel(level: LevelDefinition): LevelValidationResult {
  ...
  for (const faceId of FACE_IDS) if (!level.faces[faceId]) errors.push(`missing face definition: ${faceId}`);
  ...
  if (level.mastery.fullOrbitFaces < 2 || level.mastery.fullOrbitFaces > 6) errors.push('mastery.fullOrbitFaces must be between 2 and 6');
